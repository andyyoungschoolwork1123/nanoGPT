...
def process_date_and_price_to_tokens(df):
    ...
    # Convert the list of characters to tokens (ASCII codes for now)
    date_tokens = dates.apply(lambda x: [ord(c) for c in x])
    
    # Extract the stock prices and treat them as integers (no normalization needed)
    prices = df.iloc[:, 1].apply(lambda x: list(x.replace(' ', '')))
